// src/app/api/dxm/products/builds/route.ts
...
export const GET = apiSafe(async (request: NextRequest) => {
  if (!env.DATABASE_URL) {
    return NextResponse.json(
      { ok: true, data: getCategoryFallback("BUILDS") },
      {
        status: 200,
        headers: {
  ...
  // Query products from multiple categories for builds
  const raw = await queryAll<{
    ...
    `SELECT * FROM product_catalog WHERE category IN ($1, $2, $3) ORDER BY dxm_value_score DESC NULLS LAST LIMIT 15`,
    ["GPU", "CPU", "Laptop"]
  );
