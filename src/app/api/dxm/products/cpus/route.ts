// src/app/api/dxm/products/cpus/route.ts
...
export const GET = apiSafe(async (request: NextRequest) => {
  try {
    if (!env.DATABASE_URL) {
      console.warn("[CPU-API] DATABASE_URL not configured, using fallback data");
      return NextResponse.json(
        { ok: true, data: getCategoryFallback("CPU") },
        {
          status: 200,
          headers: {
    ...
    const finalData = normalized.length > 0 ? normalized : getCategoryFallback("CPU");
